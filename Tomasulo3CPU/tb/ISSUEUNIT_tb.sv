...
module ISSUEUNIT_tb;
    ...
    initial begin
        ...
        @(posedge clk);
        clear_ready();
        repeat (MUL_CYCLES - 1) @(posedge clk);
        ready_int    = 1'b1;
        ready_ld_buf = 1'b1;
        ...
        ready_int    = 1'b1;
        ready_ld_buf = 1'b1;
        expect_grants("first pick ld (priority=0)", 1'b0, 1'b0, 1'b0, 1'b1);
        cycle();
        clear_ready();
        ready_int    = 1'b1;
        ready_ld_buf = 1'b1;
        expect_grants("second pick int (priority flipped)", 1'b1, 1'b0, 1'b0, 1'b0);
        cycle();
        clear_ready();
        ready_int    = 1'b1;
        ready_ld_buf = 1'b1;
        expect_grants("third pick ld again", 1'b0, 1'b0, 1'b0, 1'b1);
        cycle();
        clear_ready();
