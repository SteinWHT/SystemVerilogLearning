...
module MULQ_tb;
    ...
    // Queue status
    logic                               mulq_full;
    logic                               iss_mulq_two_or_more_vacant;
    ...
    MULQ #(
        ...
        .mulq_full                   (mulq_full),
        .iss_mulq_two_or_more_vacant (iss_mulq_two_or_more_vacant)
    );
    ...
    task automatic expect_idle_outputs(input string tag);
        check_bit({tag, " issue_mul_rdy"}, issue_mul_rdy, 1'b0);
        check_bit({tag, " issue_mul"}, issue_mul, 1'b0);
        check_val({tag, " iss_rob_tag_mul"}, iss_rob_tag_mul, '0);
        check_val({tag, " iss_rd_phy_addr_mul"}, iss_rd_phy_addr_mul, '0);
        check_val({tag, " iss_rs_phy_addr_mul"}, iss_rs_phy_addr_mul, '0);
        check_val({tag, " iss_rt_phy_addr_mul"}, iss_rt_phy_addr_mul, '0);
    endtask
    ...
    task automatic expect_issue_payload(
        ...
        check_bit({tag, " issue_mul_rdy"}, issue_mul_rdy, 1'b1);
        check_bit({tag, " issue_mul"}, issue_mul, 1'b1);
        check_val({tag, " rob tag"}, iss_rob_tag_mul, rob_tag);
        check_val({tag, " rs"}, iss_rs_phy_addr_mul, rs);
        check_val({tag, " rt"}, iss_rt_phy_addr_mul, rt);
        check_val({tag, " opcode"}, iss_opcode_mul, opcode);
        check_val({tag, " rd"}, iss_rd_phy_addr_mul, rd);
        check_bit({tag, " rw"}, iss_rw_mul, rw);
    endtask
    ...
    initial begin
        `ifdef FSDB_DUMP
            $fsdbDumpfile("mulq.fsdb");
            $fsdbDumpvars(0, MULQ_tb);
        `else
            $dumpfile("mulq.vcd");
            $dumpvars(0, MULQ_tb);
        `endif
        ...
        $display("=======================================");
        $display("  MULQ Testbench Start");
        $display("=======================================");

        $display("\n[Test 1] Reset leaves queue empty and ready for dispatch");
        reset_dut();
        expect_idle_outputs("reset");
        check_bit("not full after reset", mulq_full, 1'b0);
        check_bit("two or more vacant after reset", iss_mulq_two_or_more_vacant, 1'b1);

        $display("\n[Test 2] Dispatch a ready entry and issue its payload");
        dispatch_entry(5'd3, 7'd11, 1'b1, 7'd12, 1'b1, 3'd5, 7'd20, 1'b1);
        check_bit("ready entry advertises issue_mul_rdy", issue_mul_rdy, 1'b1);
        check_bit("ready entry does not issue when disabled", issue_mul, 1'b0);
        ...
        check_bit("two entries vacant after two dispatches", iss_mulq_two_or_more_vacant, 1'b1);
        dispatch_entry(5'd10, 7'd5, 1'b1, 7'd6, 1'b1, 3'd0, 7'd12, 1'b1);
        check_bit("only one entry vacant after three dispatches", iss_mulq_two_or_more_vacant, 1'b0);
        dispatch_entry(5'd11, 7'd7, 1'b1, 7'd8, 1'b1, 3'd0, 7'd13, 1'b1);
        check_bit("full after four dispatches", mulq_full, 1'b1);
        check_bit("not two-or-more-vacant when full", iss_mulq_two_or_more_vacant, 1'b0);

        issue_and_check("issue highest-index full entry", 5'd11, 7'd7, 7'd8, 3'd0, 7'd13, 1'b1);
        check_bit("not full after one issue", mulq_full, 1'b0);
        check_bit("still fewer than two vacant after one issue", iss_mulq_two_or_more_vacant, 1'b0);

        issue_and_check("issue next highest-index full entry", 5'd10, 7'd5, 7'd6, 3'd0, 7'd12, 1'b1);
        check_bit("two vacant after two issues", iss_mulq_two_or_more_vacant, 1'b1);
        ...
        #1;
        check_bit("flush suppresses ready", issue_mul_rdy, 1'b0);
        check_bit("flush suppresses issue", issue_mul, 1'b0);
        @(posedge clk); #1;
        cdb_flush       = 1'b0;
        issue_mul_en    = 1'b0;
        dis_mul_issq_en = 1'b0;
        ...
        $display("\n=======================================");
        $display("  MULQ Testbench Done: %0d passed, %0d failed", pass_cnt, fail_cnt);
        $display("=======================================");

        if (fail_cnt == 0) begin
            $display("[PASS] MULQ_tb completed successfully");
        end else begin
            $fatal(1, "[FAIL] MULQ_tb found %0d failure(s)", fail_cnt);
        end
        $finish;
    end
