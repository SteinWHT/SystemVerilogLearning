...
// I used sw/lw for the signal name, because I designed at first supporting LW/SW only
//but it can be easily extended to support more load/store instructions by adding more opcode and some control logic.
// TODO: modify the signal name to be more generic
module LSQ
...
#(
    ...
    function automatic logic is_store(logic [OPCODE_WIDTH-1:0] op);
        instr_e instr_op;
        instr_op = instr_e'(op);
        return (instr_op == INSTR_SW || instr_op == INSTR_SD ||
                instr_op == INSTR_SB || instr_op == INSTR_SH);
    endfunction
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (issue_lsq) begin
                ...
                if (is_store(q[sel_idx].opcode)) begin
                    for (int i = 0; i < LSQ_DEPTH; i++) begin
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // Issue Store
            if (issue_lsq && is_store(q[sel_idx].opcode)) begin
