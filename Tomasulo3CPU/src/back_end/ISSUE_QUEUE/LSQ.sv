...
module LSQ #(
    ...
    lsq_entry_t q [0:LSQ_DEPTH-1];
    logic [LSQ_DEPTH-1:0] q_valid;
    logic [SAB_INDEX_WIDTH-1:0] junior_counter [0:LSQ_DEPTH-1];
        ...
        for (int i = 0; i < LSQ_DEPTH; i++) begin
            ...
            if (q_valid[i]) begin
                if (!q[i].rs_data_valid) begin
                    if (cdb_phy_reg_write   && (q[i].rs_phy_addr == cdb_rd_phy_addr))      wk_rs_rdy[i] = 1'b1;
                end
            end
        end
    end
    ...
    logic [SAB_INDEX_WIDTH-1:0] match_number [0:LSQ_DEPTH-1];
    always_comb begin
        for (int i = 0; i < LSQ_DEPTH; i++) begin
            match_number[i] = '0;
        end
            ...
            if (q_ready[i] && (entry_depth[i] < min_depth)) begin
                if (q[i].opcode == OPCODE_STORE) begin
                    for (int j = 0; j < LSQ_DEPTH; j++) begin
                        ...
                        // 2. for sw: sab is not full
                        if(!sab_full && (j != i) && (q[j].opcode == OPCODE_LOAD) && (entry_depth[j] < entry_depth[i]) 
    ...
    assign issue_lsq     = sel_valid & iss_lsq_en & ~cdb_flush;
        ...
        if (!rst_n) begin
            ...
            for (int i = 0; i < LSQ_DEPTH; i++) begin
                if (flush_mask[i] && q_valid[i]) begin
                    if (q[i].opcode == OPCODE_LOAD) begin
                        for (int j = 0; j < SAB_DEPTH; j++) begin
                            if (sab_array[j].addr == q[i].addr_offset && sab_valid[j] == 1'b1 && sab_entry_depth[j] > cdb_rob_depth) begin
            ...
            if (issue_lsq && lsb_rdy) begin
                ...
                if (q[sel_idx].opcode == OPCODE_STORE) begin
                    for (int i = 0; i < LSQ_DEPTH; i++) begin
                        ...
                        if (q[i].opcode == OPCODE_LOAD && (entry_depth[i] < entry_depth[sel_idx]) && (q[i].addr_offset == q[sel_idx].addr_offset)) begin
                            junior_counter[i] <= junior_counter[i] + 1;
                        end
                    end
                end
            ...
            if (iss_lsq_en && has_free && !cdb_flush) begin
    ...
    always_comb begin
        for (int i = 0; i < SAB_DEPTH; i++) begin
            ...
            (0) : 
            (sab_array[i].rob_tag[ROB_INDEX_WIDTH-1:0] - rob_top_ptr);
        end
    end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (sb_flush_sw) begin
                for (int i = 0; i < SAB_DEPTH; i++) begin
                    if (sab_array[i].sb_tag == sb_flush_sw_tag && sab_array[i].tag_sel == 1'b1) begin
                        sab_valid[i] <= 1'b0;
                    end
                end
            end
