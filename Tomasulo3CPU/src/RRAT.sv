...
module RRAT #(
    ...
    input logic [PHY_REG_IDX_WIDTH-1:0]     rob_commit_curr_phy_addr,
    input logic                             rob_commit,
    input logic                             rob_commit_reg_write,

    // CSR rs1 read port
    input  logic [ARCH_REG_WIDTH-1:0]       csr_rs1_arch_addr,
    output logic [PHY_REG_IDX_WIDTH-1:0]    csr_rs1_phy_addr
);
    ...
    logic [PHY_REG_IDX_WIDTH-1:0] rrat_array [ARCH_REG_COUNT];

    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            for (int i = 0; i < ARCH_REG_COUNT; i++) begin
                rrat_array[i] <= PHY_REG_IDX_WIDTH'(i);
            end
        end
        else begin
