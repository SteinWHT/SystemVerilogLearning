...
module FRAT #(
    ...
    input logic [PHY_REG_IDX_WIDTH-1:0]         rd_new_phy_addr_in,
    input logic [ARCH_REG_WIDTH-1:0]            rd_new_arch_addr_in,
    ...
    //
    input logic [ARCH_REG_WIDTH-1:0]            rd_prev_arch_addr_in,
    input logic [ARCH_REG_WIDTH-1:0]            rs1_arch_addr_in,
    input logic [ARCH_REG_WIDTH-1:0]            rs2_arch_addr_in,

    output logic [PHY_REG_IDX_WIDTH-1:0]        rd_prev_phy_addr,
    output logic [PHY_REG_IDX_WIDTH-1:0]        rs1_phy_addr,
    output logic [PHY_REG_IDX_WIDTH-1:0]        rs2_phy_addr,

    output logic full
);
    // FRAT array: maps architectural register index to physical register index
    // I don't use the circular buffer approach for FRAT
    logic [PHY_REG_IDX_WIDTH-1:0] frat_array [ARCH_REG_COUNT];
    logic [PHY_REG_IDX_WIDTH-1:0] checkpoint_frat_array [NUM_CHECKPOINT][ARCH_REG_COUNT];
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            for (int i = 0; i < ARCH_REG_COUNT; i++) begin
                frat_array[i] <= (PHY_REG_IDX_WIDTH)'(i);
            end
            ...
            if (is_branch && !full) begin
                ...
                if (dis_frat_reg_write) begin
                    checkpoint_frat_array[checkpoint_head[CHECKPOINT_PTR_WIDTH-1:0]][rd_new_arch_addr_in] 
                        <= rd_new_phy_addr_in;
                end
            end
            ...
            if (dis_frat_reg_write) begin
                frat_array[rd_new_arch_addr_in] <= rd_new_phy_addr_in;
            end
        end
    end

    assign rd_prev_phy_addr  =   frat_array[rd_prev_arch_addr_in];
    assign rs1_phy_addr      =   frat_array[rs1_arch_addr_in];
    assign rs2_phy_addr      =   frat_array[rs2_arch_addr_in];
