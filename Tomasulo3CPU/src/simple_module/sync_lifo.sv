...
module sync_lifo #(
    ...
    logic [DATA_WIDTH-1:0] lifo_array [0:DEPTH-1];
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if(UNDERFLOW_PROTECT && empty) begin
                ...
                last_pop_data <= lifo_array[(top_ptr - 1) & (DEPTH - 1)]; // store the popped data for potential underflow protection
    ...
    assign empty = ((top_ptr[PTR_WIDTH-1:0] == top_entry_index[PTR_WIDTH-1:0]) && (top_ptr[PTR_WIDTH] == !top_entry_index[PTR_WIDTH])); // LIFO is empty when top pointer is at 0
