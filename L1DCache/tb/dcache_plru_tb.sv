...
module dcache_plru_tb;
    ...
    initial begin
        ...
        // Cold: all ways equal → victim 0 (tree favors way 0 when state==0)
        check_victim("reset", 2'd0);

        // 16-step script: touch 3,1,0,2,3,3,1,2,0,1,2,3,0,0,1,1
        ...
        if (errors == 0)
            $display("dcache_plru_tb: PASS");
        else
            $display("dcache_plru_tb: FAIL (%0d errors)", errors);
