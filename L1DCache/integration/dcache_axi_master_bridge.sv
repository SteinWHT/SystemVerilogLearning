// Translate the D-cache's single-word req/ack backing-memory port to AXI4.
//
...
// full AXI4 and can share the existing fabric with another master such as DMA.

module dcache_axi_master_bridge #(
    parameter int unsigned MEM_IDX_WIDTH = dcache_pkg::MEM_IDX_BITS,
    ...
    initial begin
        if (DATA_WIDTH < 8 || (DATA_WIDTH % 8) != 0)
            $fatal(1, "dcache_axi_master_bridge: invalid DATA_WIDTH");
        if ((1 << BYTE_LSB) != STRB_WIDTH)
            $fatal(1, "dcache_axi_master_bridge: DATA_WIDTH/8 must be power of two");
        if (MEM_IDX_WIDTH + BYTE_LSB > ADDR_WIDTH)
            $fatal(1, "dcache_axi_master_bridge: AXI address is too narrow");
        if (ID_WIDTH == 0)
            $fatal(1, "dcache_axi_master_bridge: ID_WIDTH must be >= 1");
    end
