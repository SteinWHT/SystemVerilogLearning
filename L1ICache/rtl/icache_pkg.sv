// L1 I-cache parameters and helpers
// Cache configuration:
//   16 KiB, 4-way set-associative, 64-byte lines, read-only
//   CPU fetch granularity is a 32-bit instruction (RISC-V default)
//   A line is refilled as an 8-beat burst on a 64-bit memory bus
...
package icache_pkg;
    ...
    // CPU fetch granularity (one instruction per request by default)
    localparam int unsigned INSTR_BITS    = 32;
    localparam int unsigned INSTR_BYTES   = INSTR_BITS / 8;         // 4
    localparam int unsigned FETCH_WIDTH   = INSTR_BITS;

    // Cache line
    localparam int unsigned LINE_BYTES    = 64;
    localparam int unsigned LINE_BITS     = LINE_BYTES * 8;         // 512
    localparam int unsigned INSTRS_PER_LINE = LINE_BYTES / INSTR_BYTES; // 16

    // Backing memory bus (same 64-bit word path as L1DCache for AXI reuse)
    localparam int unsigned MEM_WORD_BITS  = 64;
    localparam int unsigned MEM_WORD_BYTES = MEM_WORD_BITS / 8;     // 8
    localparam int unsigned MEM_WORDS_PER_LINE = LINE_BYTES / MEM_WORD_BYTES; // 8
    ...
    //   line-offset = | instr-select | byte-in-instr |
    localparam int unsigned INSTR_OFF_BITS = $clog2(INSTR_BYTES);   // 2  addr[1:0]
    localparam int unsigned INSTR_SEL_BITS = $clog2(INSTRS_PER_LINE); // 4 addr[5:2]
    ...
    // Backing memory: word-addressable 64-bit SRAM (same model as L1DCache).
    localparam int unsigned MEM_BYTE_OFF_BITS = $clog2(MEM_WORD_BYTES); // 3
    ...
    typedef logic [INSTR_SEL_BITS-1:0]  instr_sel_t;
    typedef logic [NUM_WAYS-1:0]        way_mask_t;
    typedef logic [LINE_BITS-1:0]       cache_line_t;
    typedef logic [INSTR_BITS-1:0]      cache_instr_t;
    typedef logic [FETCH_WIDTH-1:0]     fetch_data_t;
    ...
    typedef logic [MEM_WORD_BITS-1:0]   mem_word_t;
    ...
    function automatic instr_sel_t addr_to_instr_sel(input cache_addr_t addr);
        return addr[INSTR_OFF_BITS +: INSTR_SEL_BITS];
    endfunction
    ...
    function automatic mem_idx_t addr_to_line_base_widx(input cache_addr_t addr);
        ...
        return base[MEM_BYTE_OFF_BITS +: MEM_IDX_BITS];
    endfunction

    function automatic cache_way_t plru_victim(input plru_state_t state);
        if (!state[0])
            plru_victim = state[1] ? 2'd1 : 2'd0;
        else
            plru_victim = state[2] ? 2'd3 : 2'd2;
    endfunction
    ...
    function automatic cache_instr_t line_get_instr(
        input cache_line_t line,
        input instr_sel_t  sel
    );
        return line[int'(sel)*INSTR_BITS +: INSTR_BITS];
    endfunction
